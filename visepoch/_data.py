...
Contains functions for handling EPOCH simulation data. These should not be
...
def x_sub_sample(grid, x_select):
    ...
    x_select : tuple
        ...
        sample in SI units ``(xmin, xmax)`` relative to minimum boundary. Value
        of ``None`` sets it to minimum or maximum border.
