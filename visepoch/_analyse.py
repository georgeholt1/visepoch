...
Contains functions for analysing EPOCH simulation data. These should not be
accessed directly as they are imported by the Simulation class.
"""
import numpy as np
import os
...
def calculate_electron_energy_avg_std(
    ...
    x_select : tuple, optional
        ...
        sub-sample in SI relative window units ``(xmin, xmax)``. A value of
        ``None`` sets the sub-sample to be the minimum or maximum x-border.
    ...
    force : bool, optional
        Set to ``True`` to force the get, even if data has already been got.
        Useful if `x_select` changes. Defaults to ``False``.
    ...
    | - Particle ``position``, ``weight`` and ``energy`` dumped for
      ``high_gamma`` subset.  
...
def calculate_electron_charge(
    ...
    x_select : tuple, optional
        2-tuple with float values specifying the x-position of particles to
        sub-sample in SI relative window units ``(xmin, xmax)``. A value of
        ``None`` sets the sub-sample to be the minimum of maximum x-border.
        E.g. ``(5e-6, 20e-6)`` will sample the particles with position values
        between 5 and 20 microns in the window units.
        Defaults to ``(None, None)``, which performs no sub-sampling.
    force : bool, optional
...
def calculate_electron_energy_spectrum(
    ...
    subset : {'all', 'high_gamma'}
        The subset of electron to calculate the spectrum for. Can be ``'all'``
        or ``'high_gamma'``. Defaults to ``'all'``.
    x_select : tuple, optional
        2-tuple with float values specifying the x-position of particles to
        sub-sample in SI relative window units ``(xmin, xmax)``. A value of
        ``None`` sets the sub-sample to be the minimum of maximum x-border.
    ...
    | Input deck requirements:
    | - If subset is ``high_gamma``, then a subset of the ``electron`` species
      called ``high_gamma``.
    
    | - Particle ``position``, ``weight`` and ``energy`` dumped for the
      ``electron`` species if subset is ``all``, or for the ``high_gamma``
      subset if ``subset`` is ``high_gamma``.
...
def calculate_back_of_bubble_position_velocity(
    ...
    :meth:`visepoch.simulation.Simulation.plot_back_of_bubble_helper` to find
    the time region over which the measurements are valid.
    ...
    t_start : float, optional
        ...
        velocity. Defaults to ``None``, which selects the earliest simulation
        time.
    t_end : float, optional
        Upper boundary of time window within which to make measurements of the
        velocity. Defaults to ``None``, which selects the latest simulation
        time.
    force : bool, optional
        Set to ``True`` to force the get, even if data has already been got.
        Useful if `x_select` changes. Defaults to ``False``.
    ...
    if self.back_of_bubble_velocity is not None:
        if not force:
            return (
                self.back_of_bubble_position,
                self.back_of_bubble_velocity,
                self.back_of_bubble_t_mid
            )
